...
const kadınroleID = await data.fetch(`kadınRol.${message.guild.id}`);
const yetkiliroleID = await data.fetch(`yetkiliRol.${message.guild.id}`);
const kayıtsızroleID = await data.fetch(`kayıtsızRol.${message.guild.id}`);
...
**Örnek:**
\`\`\`kadın @etiket
kadın @etiket Lady 13\`\`\``)).then(a => a.delete({timeout: 10000}));
...
${message.author} **Kayıt sayın:** \`!!kayıtsayısı yazarak görebilirsin!\`
